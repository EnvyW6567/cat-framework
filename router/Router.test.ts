...
import { HttpContentTypeExt } from '../http/type/HttpContentType';
...
describe('Router 테스트', () => {
    ...
    beforeEach(() => {
        process.env.STATIC_FILE_PATH = 'test/public';
        process.env.VIEW_FILE_PATH = 'test/public/view';
        router = new Router();
        mockResponse = {
    ...
    const testStaticFile = async (url: string, ext: HttpContentTypeExt, expectedPath: string) => {
        const mockFileContent: string = 'mock file content';

        (fs.readFile as jest.Mock).mockResolvedValue(mockFileContent as never);
    ...
    describe('라우팅에 실패해야함 - ', () => {
        it('없는 path 요청 시', () => {
            ...
            expect(async () => await router.handle(mockRequest, mockResponse, () => {
    ...
    describe('HTTP 메서드별 핸들러 동작 테스트', () => {
        it('GET 요청 핸들링에 성공해야함', async () => {
            const mockHandler = jest.fn((req: HttpRequest, res: HttpResponse) => Promise.resolve());
    ...
    describe('정적 파일 요청 핸들링 테스트', () => {
        it('.css 파일 요청 핸들링에 성공해야함', async () => {
            ...
            await testStaticFile(staticPath, '.css', path.join('test/public', staticPath));
        });

        it('.js 파일 요청 핸들링에 성공해야함', async () => {
            const staticPath = '/js/test.js';

            await testStaticFile(staticPath, '.js', path.join('test/public', staticPath));
        });

        it('.html 파일 요청 핸들링에 성공해야함', async () => {
            const staticPath = '/test.html';

            await testStaticFile(staticPath, '.html', path.join('test/public', '/view', staticPath));
        });
    });
});